...
```bash
node docs/code/build-polygon-payload.js ETHDenver2026Buidlathon/ChainRoute-Forge/Examples/HypotheticalPaintingRevised/<payload-file>.json
```
...
Run all commands from the **ChainRoute repo root**. This example lives at `ETHDenver2026Buidlathon/ChainRoute-Forge/Examples/HypotheticalPaintingRevised/`; use that path when scripts expect an example directory (e.g. for payload JSONs or chain-manifest).
