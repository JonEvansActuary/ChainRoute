...
node docs/code/post-support-to-arweave.js \
  docs/examples/HypotheticalPainting/supports/UV_Analysis_Picasso_Hypothetical.png \
  --genesis <GENESIS_HASH> --key "$ARWEAVE_KEY"
# → Capture as ID_UV

node docs/code/post-support-to-arweave.js \
  docs/examples/HypotheticalPainting/supports/Spectral_Analysis_Report_v1.pdf \
  --genesis <GENESIS_HASH> --key "$ARWEAVE_KEY"
# → Capture as ID_SPECTRAL

node docs/code/post-support-to-arweave.js \
  docs/examples/HypotheticalPainting/supports/Notary_Seal_Paris.png \
  --genesis <GENESIS_HASH> --key "$ARWEAVE_KEY"
# → Capture as ID_NOTARY

node docs/code/post-support-to-arweave.js \
  docs/examples/HypotheticalPainting/supports/ArtSecure_Cert_12345.pdf \
  --genesis <GENESIS_HASH> --key "$ARWEAVE_KEY"
...
Create a file with the four support IDs (replace with your actual IDs):

```bash
# Replace the four IDs with the ones from Step 2
...
node docs/code/post-provenance-blob-to-arweave.js \
  <GENESIS_HASH> \
  docs/examples/HypotheticalPainting/1-authentication.json \
...
| 2    | 4 × Arweave IDs   | Build `event1-supports.json` and blob |
